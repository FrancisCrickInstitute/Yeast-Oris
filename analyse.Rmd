...
2. We mapped the ROC PWM to the OOR sequences.
3. We removed OORs with zero ROC hits and multiple maximum scoring ROC hits.
4. We annotated the ROC elements for each OOR. We labelled the maximum scoring ROC
   hit as the A-element and all ROCs downstream and in
...
12. We calculated the mean distance from the A-element to the first
    B-element.
13. We created spreadsheets containing the annotated ROCs and all ROC hits. 
...
We read in the probability ROC PWM matrix and transformed it into a PWM
...
```{r roc_pwm}
pwm_file <- file.path( "data", "ROC_PWM_text.txt" )
ROC_motif <- read.delim( file = pwm_file, sep = "\t" ) %>%
    t( )
ROC_motif
ROC_pwm = toPWM( ROC_motif )
maxScore( ROC_pwm$pwm )
```
...
ROC_matches_raw <- map( 1:length( target_seqs ), function( seq_i ) {
    hit_f <- matchPWM( ROC_pwm$pwm, target_seqs[[ seq_i ]], with.score = TRUE, min.score = 0 )
    if( length( hit_f ) > 0 )
        mcols( hit_f )$strand <- 1
    hit_r <- matchPWM( reverseComplement( ROC_pwm$pwm ), target_seqs[[ seq_i ]],
...
OOR_regions2names_df <- data.frame( fasta_name = names( ROC_matches_raw ) ) %>%
    left_join( OOR_bed_df, by = "fasta_name" )
names( ROC_matches_raw ) <- OOR_regions2names_df$name
```

# Annotate A and B elements

For each OOR we identified the A-element as the highest scoring ROC match. We
labelled all downstream elements in the opposite orientation to the
A-element as B-elements.

We removed two OORs with multiple maximum ROC matches since we could not
identify the true ROC A-element.

We also removed 18 OORs with zero B-elements.

```{r filter_multi_max_rocs}
max_filter <- ROC_matches_raw %>%
    map( function( df ) sum( df$score == max( df$score ) ) ) %>%
    unlist( )
ROC_matches_raw <- ROC_matches_raw[ max_filter == 1 ] %>%
    map( mutate, midpoint = (start + ((end-start)/2)) )
```
```{r annotated_elements}
ROC_matches <- ROC_matches_raw %>%
    map( function( df ) {
        a_element <- df %>%
...
b_element_count <- ROC_matches %>%
    map( function( df ) sum( df$element == "B-element" ) ) %>%
   unlist( )
ROC_matches <- ROC_matches[ b_element_count > 0 ]
```
...
boundary). The ROC A and B-elements for ARS416 are shown in the table
below.
...
outer_boundary <- 400
ROC_matches$ARS416 %>% DT::datatable( rownames = FALSE )
...
```{r dist_to_first_b}
belement_dist_plotdat <- ROC_matches %>%
    map( filter, element == "B-element" ) %>%
    map( filter, score >= score_thr_80 ) %>%
...
We wrote A and B-element annotated ROC matches and raw ROC matches to
...
shown below. **all_ROC_hits** contains all reported ROC matches with a
subset of the columns described below.

The file can be downloaded [here](AB-element_ROC_OOR_data.xlsx)

* **OOR_id** - ARSxxx ID from OriDB.
* **start** - start position of ROC hit.
* **end** - end position of ROC hit.
* **width** - width of ROC hit.
* **score** - ROC match score.
* **stand** - orientation of ROC hit.
* **seq_length** - Length of OOR sequence.
* **element** - ROC is an A or B element.
...
```{r output}
dbase_l <- list( 'AB-elements' = ROC_matches %>%
                     bind_rows( .id = "OOR_id" ) %>%
                     mutate( pass_80pc_thr = score >= score_thr_80 ),
                 all_ROC_hits = ROC_matches_raw %>%
                     bind_rows( .id = "OOR_id" ) )

dbase_l %>%
    write.xlsx( "AB-element_ROC_OOR_data.xlsx" )
```
...
```{r cluster_oor}
max_score_b_dist_ord <- ROC_matches %>%
    map( filter, element == "B-element" ) %>%
    map( filter, score == max( score ) ) %>%
    map( filter, row_number( ) == 1 ) %>%
...
mean of the ROC match scores per base for all B-elements across all aligned
...
ROC_matches_flipped <- ROC_matches %>%
    map( function( df ) {
        a_element_df <- df %>%
...
max_b_element_coor <- ROC_matches_flipped %>%
    map( function( df ) max( df$end ) ) %>%
    unlist( ) %>%
    max( )

roc_mat <- matrix( 0, nrow = length( ROC_matches_flipped ), ncol = max_b_element_coor )

coors_vec <- map( ROC_matches_flipped, function( df ) {
    df %>%
        rowwise( ) %>%
...
score_vec <- map( ROC_matches_flipped, function( df ) {
    rep( df$score, each = 17 )
} )

OOR_ind <- sapply( 1:length( ROC_matches_flipped ), function( ind ) {
    rep( ind, ( nrow( ROC_matches_flipped[[ ind ]] ) * 17 ) )
} ) %>%
    unlist( )
